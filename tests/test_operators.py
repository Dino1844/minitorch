...
def test_distribute(x: float, y: float, z: float) -> None:
    ...
    """
    assert_close (mul(z, minitorch.operators.add(x, y)), minitorch.operators.add(mul(x, z), mul(y, z)))
...
def test_other(a: float, b:float) -> None:
    ...
    """
    assert minitorch.operators.is_close(a, b) == (abs(a - b) < 1e-2)
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    combined = minitorch.operators.sum([x + y for x, y in zip(ls1, ls2)])
    assert_close(sum1 + sum2, combined)
